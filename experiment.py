...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Benchmarking and Graphing Tool with Improved Axis Label Handling')
    ...
    # New arguments for figure size and label rotation
...
def main():
    ...
    if args.scale and not args.scale_x and not args.scale_y:
        # 2D plot
        scales = args.scale
        ...
        print("Invalid combination of scale parameters. For 2D plots, provide '--scale'. For 3D plots, provide both '--scale_x' and '--scale_y'.")
        sys.exit(1)
